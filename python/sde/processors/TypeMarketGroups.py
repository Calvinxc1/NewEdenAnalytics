...
class TypeMarketGroupsProcessor(SdeProcessor):
    ...
    def update_market(self):
        """ Constructs and updates the Market Group path data
        
        This function calls the constructor for the Market Groups path,
        then calls the inserter for the path data.
        """
        mkt_grp_data = self.parse_market_path()
        self.update_market_groups(mkt_grp_data)
    
    def parse_market_path(self):
        """ Parses the Market Groups to construct their path
        
        Semi-recursively parses through the Market Groups to construct a
        flat-form of their path from thheir root class.
        
        Returns
        -------
        DataFrame
            Pandas DataFarme consisting of the flat-form market group path
            and the market group's id number
        """
    ...
    def _parse_market(self, mkt_grp_ids):
        """ Captures market group data from Maria for parse_market_path
        
        Working as a sub-process of the parse_market_path method, this
        captures the market data and returns it to help construct the
        flat-form path value.
        
        Returns
        -------
        DataFrame
            Pandas DataFrame consisting of market group data for parsing/iterating
            via the parse_market_path method
        """
    ...
    def update_market_groups(self, mkt_grp_data):
        """ Inserts market group path information
        
        Takes the path inforamtion generated by parse_market_path and
        inserts it into Maria
        
        Parameters
        ----------
        mkt_grp_data: DataFrame
            A DataFrame with the columns ['path', 'market_group_id'] for
            insertion into Maria
        """
